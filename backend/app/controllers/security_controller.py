...
from app.models import Users, UsersRoles
...
def get_role(user: Users):
    return user.users_roles[0].role.title
...
def register_user_tenant_juridical(db: Session, email: str, password: str, repeat_password: str):
    user = get_user(db, email)
    if not user:
